...
impl Matcher for CharMatcher {
    ...
    ) -> Result<MatchResult, crate::compare::MatchError> {
        match self {
            Self::Exact(char) => {
...
impl fmt::Display for ExprParseError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        if self.message.is_some() {
            write!(f, "Unexpected {} at index {}: {}", char_string.bold(), self.index, self.message.as_ref().unwrap())
        } else {
            write!(f, "Unexpected {} at index {}", char_string.bold(), self.index)
        }
    }
}
...
mod tests {
    ...
    fn exact_char_matcher_works() {
        ...
        assert_eq!(matcher.compare(&mut seq_view("a", &mut strbuf), &mut str, &MatcherContext::new(&(), false)), Ok(MatchResult::Match));
		assert_eq!(str, String::from("a"));

        let mut str = String::new();
        assert_eq!(matcher.compare(&mut seq_view("abc", &mut strbuf), &mut str, &MatcherContext::new(&(), false)), Ok(MatchResult::Match));
		assert_eq!(str, String::from("a"));

        let mut str = String::new();
        assert_eq!(matcher.compare(&mut seq_view("fsfd", &mut strbuf), &mut str, &MatcherContext::new(&(), false)), Err(MatchError::simple("'a'".to_string(), 0)));


        let mut str = String::new();
        assert_eq!(matcher.compare(&mut seq_view("", &mut strbuf), &mut str, &MatcherContext::new(&(), false)), Err(MatchError::simple("'a'".to_string(), 0)));
    }
    ...
    fn range_char_matcher_works() {
        ...
        let mut str = String::new();
        assert_eq!(matcher.compare(&mut seq_view("0", &mut strbuf), &mut str, &MatcherContext::new(&(), false)), Err(MatchError::simple("char in range '1' to '3'".to_string(), 0)));

        let mut str = String::new();
        assert_eq!(matcher.compare(&mut seq_view("1", &mut strbuf), &mut str, &MatcherContext::new(&(), false)), Ok(MatchResult::Match));
		assert_eq!(str, String::from("1"));

        let mut str = String::new();
        assert_eq!(matcher.compare(&mut seq_view("2", &mut strbuf), &mut str, &MatcherContext::new(&(), false)), Ok(MatchResult::Match));
		assert_eq!(str, String::from("2"));

        let mut str = String::new();
        assert_eq!(matcher.compare(&mut seq_view("3", &mut strbuf), &mut str, &MatcherContext::new(&(), false)), Ok(MatchResult::Match));
		assert_eq!(str, String::from("3"));

        let mut str = String::new();
        assert_eq!(matcher.compare(&mut seq_view("4", &mut strbuf), &mut str, &MatcherContext::new(&(), false)), Err(MatchError::simple("char in range '1' to '3'".to_string(), 0)));
    }
    ...
    fn can_match_exact() {
        ...
        assert_eq!(matcher.compare("abedefg"), Err(format!("(expr:2) Expected {}", "'c'".bold())), "'abc' should not match in 'abedefg'");
    }
    ...
    fn can_repeat_single() {
        ...
        assert_eq!(matcher.compare("cgfds"), Err(format!("(expr:0) Expected {}", "'a'".bold())), "'ab*' should not match in 'cgfds'");
    }
    ...
    fn can_repeat_multiple() {
        ...
        assert_eq!(matcher.compare("bcbcbcbcefdfs"), Err(format!("(expr:0) Expected {}", "'a'".bold())), "'a(bc)*' should not match 'bcbcbcbcefdfs'");
    }
    ...
    fn can_repeat_multiple_plus() {
        ...
        assert_eq!(matcher.compare("abcbcbcbcefdfs"), Ok(String::from("abcbcbcbc")), "'a(bc)+' should match first 9 chars in 'abcbcbcbcefdfs'");
        assert_eq!(matcher.compare("bcbcbcbcefdfs"), Err(format!("(expr:0) Expected {}", "'a'".bold())), "'a(bc)+' not match 'bcbcbcbcefdfs'");
        assert_eq!(matcher.compare("aefdfs"), Err(format!("(expr:1) Expected {}", "'b'".bold())), "'a(bc)+' should not match 'aefdfs'");
    }
    ...
    fn optional_works() {
        ...
        assert_eq!(matcher.compare("gdfefdfs"), Err(format!("(expr:0) Expected {}", "'a'".bold())), "'a(bc)?' not match 'gdfefdfs'");
    }
    ...
    fn choice_works() {
        ...
        assert_eq!(
            ...
            Err(format!("(expr:1) Expected one of: {}, {}", "'b'".bold(), "'c'".bold())),
    ...
    fn escape_works() {
        ...
        assert_eq!(matcher.compare("(([a"), Err(format!("(expr:3) Expected {}", "'*'".bold())), "'\\(\\(\\[\\*' should not match '(([a'");
    }
    ...
    fn escape_in_choice_works() {
        ...
        assert_eq!(
            ...
            Err(format!("(expr:0) Expected one of: {}, {}, {}", "'a'".bold(), "'-'".bold(), "']'".bold())),
    ...
    fn ranges_work() {
        ...
        assert_eq!(matcher.compare("A"), Err(format!("(expr:0) Expected {}", "char in range 'a' to 'z'".bold())), "'[a-z]' should not match 'A'");
        assert_eq!(matcher.compare("-"), Err(format!("(expr:0) Expected {}", "char in range 'a' to 'z'".bold())), "'[a-z]' should not match '-'");
    }
    ...
    fn multiple_ranges_work() {
        ...
        assert_eq!(
            matcher.compare("3"),
            Err(format!("(expr:0) Expected one of: {}, {}", "char in range '1' to '2'".bold(), "char in range '4' to '5'".bold())),
